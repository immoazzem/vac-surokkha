...
import { sendOTP, verifyOTP } from './../../services/enroll';
import { regenerateID } from './../../services/otp';
...
const EnrollFormOtpVerify = (props) => {
    ...
    useEffect(() => {
        ...
        if (props.location?.state.tempRegistrationId) {
            if (!props.location?.state.enrollmentInfo.mobile) {
                ...
                regenerateID(postData).then((response) => {
    ...
    const onOTPVerifyClickHandler = (data) => {
        ...
        verifyOTP(postData).then((response) => {
            if (response.response.result) {
                setOtpVerified(true);
                setErrorMsg(false);
    ...
    const onOTPResend = (data) => {
        ...
        sendOTP(postData).then((response) => {
            if (response.response.result) {
                                ...
                                <div className="col-xl-4">
                                    <div className="form-group fv-plugins-icon-container">
                                        ...
                                        <div className="input-group input-radious">
                                            <input type="text" value={props.location.state.enrollmentInfo.mobile} className="form-control" readOnly />
                                        </div>
                                    </div>
                                </div>
                            </div>
